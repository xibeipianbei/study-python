...
class  Dict(dict):
	...
	def create_engine(user, password, database, host='127.0.0.1',port=3306, **kw):
		...
		engine = _Engine(lambda: mysql.connetor.connect(**params))
		# test connection...
	...
	def with_connection(func):
		'''
		Dectorator for reuse connection.
	...
	def with_transaction(func):
		'''
		A decorator that makes function around transactions
		...
		>>> select_one('select_one * from user where id=?',8080).passwd
		u'JULIA'
	...
	def select_one(sql, *args)
		...
		>>> u2 = dict(id=100, name='Sarah', email='sarah@test.org',passwd='ABC-12345', last_modified=time.time())
		>>> insert('user', **u1)
		1
		>>> insert('user', **u2)
		1
	...
	def select_int(sql, *args):
		'''
		Excute select SQL and expected one int and only one int result:
	...
	def  select(sql, *args):
		'''
		Excute select SQL and return list or empty list if no result.

		>>> u1 = dict(id=200, name='Wall.E', email='wall.e@test.org', passwd='back-to-earth', last_modified=time.time())
		>>> u2 = dict(id=200, name='Eva', email='eva@test.org', passwd='back-to-earth', last_modified=time.time())
		>>> insert('user', **u1)
		1
		>>> insert('user', **u2)
		1
	...
	if __name__ == '__main__':
		logging.basicConfig(level=login.DEBUG)
		create_engine('www-data', 'www-data', 'studyPython')
